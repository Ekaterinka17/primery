...
### Настройки Git
Чтобы Git понимал — кто вносит конечные изменения в удаленный репозиторий, необходима настройка (на уровне пользователя). Без нее Git работать не будет (см. подраздел **FAQ**).

Настройка имени пользователя может осуществляться как *для каждого репозитория на компьютере*, так и *для одного репозитория*. Выполним глобальную настройку для каждого репозитория на компьютере.
