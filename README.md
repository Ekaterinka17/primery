...
Для клонирования удаленного репозитория создадим сперва главную папку проекта, чтобы мы знали где все хранится. В моем случае это папка **Git_proect**.

Далее выполняем команды через терминал Git Bash:

- `cd <путь к папке>` — переход в папку через путь **C:/..** (у каждого он свой, например у меня это `cd C:/Users/edurnykh/Git_proect`), слэши обязательно с наклоном вправую сторону.
- `git clone <ссылка на удаленный репозиторий>` — происходит клонирование удаленного репозитория на ПК. Ссылку берем в GitHub с адресом https в меню **Code**→**Local** (см. рис. 4 настоящей инструкции).

Откроем на ПК папку **Git_proect** и увидим в ней вторую склонированную папку **primery**. Вторая папка — наименование проекта\удаленного репозитория на GitHub. Перейдем в эту папку с помощью команды:

- `cd <имя папки>` — переходим в папку репозитория (у каждого он свой, например у меня это `cd primery`).
...
Список команд в Git обширен, его можно подробно изучить на просторах интернета. Здесь представлены самые распространенные и часто используемые в работе:
...
7. `git checkout -b <vetka>` - одновременное создние и переключение в ветку с имененем *vetka*.
...
11. `git push -u origin <vetka>` – отправляет изменения на удаленный репозиторий.
