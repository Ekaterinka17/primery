...
- На вкладке *Source Control* отображаются файлы с изменениями, которые можно добавлять в индекс и коммитить. Для этого заполняем поле коментарием (1) и нажимаем кнопку **Commit** (2) (описанные действия аналоги комманд `git add .` и `git commit -m "коммит" в терминале Git Bash`).
...
- На вкладке *Source Control Graph* можно отслеживать на каком коммите находится удаленная и локальная ветки.
   ...
   - Оранжевое выделение показывает главную ветку, старт создания проекта.
