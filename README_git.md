...
Описание команд, выполняемых в терминале *Git Bash*, для работы с локальным репозиторием см. в подразделе **Команды Git для работы в терминале** инструкции по [ссылке](https://github.com/Ekaterinka17/primery/blob/main/README.md). В подразделе **Работа c Git через интерфейс VS Code** показаны альтернативные команды Git, которые можно выполнять с помощью функционала VS Code.
