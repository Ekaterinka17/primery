...
2. Нажмите кнопку **Resolve conflicts**.
   ...
   Для решения конфликта выберите изменения между строкой 6 и 12, оставив нужную, повторяющиеся предложения удалите, скобочки <<<<<<< >>>>>>> и значки ======= уберите.
...
2. Откройте конфликтный файл. Помечается в проекте восклицательным знаком.
   ...
   Для решения конфликта выберите изменения между зеленым и синим блоком, либо объедините их любым удобным способом, скобочки <<<<<<< >>>>>>> и значки ======= уберите.
...
3. Нажмите кнопку **Resolve in Merge Editor**.
   ...
   Проверьте блок _Result_, внесите окончательные правки перед слиянием.
